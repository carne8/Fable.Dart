...
module Future =
    ...
    type FutureBuilder() =
        member _.Bind(p: Future<'T1>, f: 'T1 -> Future<'T2>): Future<'T2> = p |> bind f
        member _.For(p: Future<'T1>, f: 'T1 -> Future<'T2>): Future<'T2> = p |> bind f
        [<Emit("$1.then(() => $2)")>]
        ...
        member _.BindReturn(y: Future<'T1>, f) = map f y
